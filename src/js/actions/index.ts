        ...
        return axios
            ...
            .then(
                axios.spread((gitHubNotifications, ...entAccNotifications) => {
                    ...
                    if (allNotifications[0].notifications.length === 0){
                        dispatch({
                        ...
                        let count = 0;
                        for (let i = 0; i < allNotifications[0].notifications.length; i++) {
                            const notification = allNotifications[0].notifications[i];
                            getPullRequestInfo(notification.subject.type, notification.subject.url, settings.patToken).then((info) => {
...
export async function getPullRequestInfo(type, url, token) {
    ...
    const response = await apiRequestAuth(url, Methods.GET, token, {});
    ...
    if (data.state === "open" && data.merged === false) {
        const response2 = await apiRequestAuth(url + "/reviews", Methods.GET, token, {});
